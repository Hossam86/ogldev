...
namespace ogl
{
	...
	bool
	BasicMesh::LoadMesh(const string& Filename)
	{
	...
	bool
	BasicMesh::InitFromScene(const aiScene* pScene, const string& Filename)
	{
	...
	void
	BasicMesh::CountVerticesAndIndices(const aiScene* pScene, unsigned int& NumVertices, unsigned int& NumIndices)
	{
	...
	void
	BasicMesh::ReserveSpace(unsigned int NumVertices, unsigned int NumIndices)
	{
	...
	void
	BasicMesh::InitAllMeshes(const aiScene* pScene)
	{
		for (unsigned int i = 0; i < m_Meshes.size(); i++)
		{
	...
	void
	BasicMesh::InitSingleMesh(uint MeshIndex, const aiMesh* paiMesh)
	{
	...
	void
	BasicMesh::InitSingleMeshOpt(uint MeshIndex, const aiMesh* paiMesh)
	{
		const aiVector3D Zero3D(0.0f, 0.0f, 0.0f);
	...
	void
	BasicMesh::OptimizeMesh(int MeshIndex, std::vector<uint>& Indices, std::vector<Vertex>& Vertices)
	{
	...
	bool
	BasicMesh::InitMaterials(const aiScene* pScene, const string& Filename)
	{
	...
	void
	BasicMesh::LoadTextures(const string& Dir, const aiMaterial* pMaterial, int index)
	{
	...
	void
	BasicMesh::LoadDiffuseTexture(const string& Dir, const aiMaterial* pMaterial, int MaterialIndex)
	{
	...
	void
	BasicMesh::LoadDiffuseTextureEmbedded(const aiTexture* paiTexture, int MaterialIndex)
	{
	...
	void
	BasicMesh::LoadDiffuseTextureFromFile(const string& Dir, const aiString& Path, int MaterialIndex)
	{
		string p(Path.data);
	...
	void
	BasicMesh::LoadSpecularTexture(const string& Dir, const aiMaterial* pMaterial, int MaterialIndex)
	{
	...
	void
	BasicMesh::LoadSpecularTextureEmbedded(const aiTexture* paiTexture, int MaterialIndex)
	{
	...
	void
	BasicMesh::LoadSpecularTextureFromFile(const string& Dir, const aiString& Path, int MaterialIndex)
	{
		string p(Path.data);
	...
	void
	BasicMesh::LoadColors(const aiMaterial* pMaterial, int index)
	{
	...
	void
	BasicMesh::PopulateBuffersNonDSA()
	{
	...
	void
	BasicMesh::Render(IRenderCallbacks* pRenderCallbacks)
	{
		glBindVertexArray(m_VAO);
	...
	void
	BasicMesh::Render(unsigned int DrawIndex, unsigned int PrimID)
	{
		glBindVertexArray(m_VAO);
	...
	void
	BasicMesh::Render(unsigned int NumInstances, const Matrix4f* WVPMats, const Matrix4f* WorldMats)
	{
	...
	void
	BasicMesh::GetLeadingVertex(uint DrawIndex, uint PrimID, Vector3f& Vertex)
	{
