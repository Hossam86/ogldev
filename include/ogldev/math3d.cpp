...
namespace ogl
{
	...
	void
	Vector3f::Rotate(float Angle, const Vector3f& V)
	{
	...
	void
	Matrix4f::InitScaleTransform(float ScaleX, float ScaleY, float ScaleZ)
	{
		m[0][0] = ScaleX;
		m[0][1] = 0.0f;
		m[0][2] = 0.0f;
	...
	void
	Matrix4f::InitScaleTransform(float Scale)
	{
		InitScaleTransform(Scale, Scale, Scale);
	}

	void
	Matrix4f::InitScaleTransform(const Vector3f& Scale)
	{
		InitScaleTransform(Scale.x, Scale.y, Scale.z);
	}

	void
	Matrix4f::InitRotateTransform(float RotateX, float RotateY, float RotateZ)
	{
		Matrix4f rx, ry, rz;
	...
	void
	Matrix4f::InitRotateTransformZYX(float RotateX, float RotateY, float RotateZ)
	{
		Matrix4f rx, ry, rz;
	...
	void
	Matrix4f::InitRotateTransform(const Vector3f& Rotate)
	{
	...
	void
	Matrix4f::InitRotateTransform(const Quaternion& quat)
	{
	...
	void
	Matrix4f::InitTranslationTransform(const Vector3f& Pos)
	{
	...
	void
	Matrix4f::InitCameraTransform(const Vector3f& Pos, const Vector3f& Target, const Vector3f& Up)
	{
	...
	void
	Matrix4f::InitPersProjTransform(const PersProjInfo& p)
	{
	...
	void
	Matrix4f::InitOrthoProjTransform(const OrthoProjInfo& p)
	{
		float l = p.l;
		float r = p.r;
		float b = p.b;
	...
	void
	Matrix4f::CalcClipPlanes(Vector4f& l, Vector4f& r, Vector4f& b, Vector4f& t, Vector4f& n, Vector4f& f) const
	{
