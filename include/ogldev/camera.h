...
namespace ogl
{
	...
	public:
		...
		BasicCamera(
			const OrthoProjInfo& orthoProjInfo,
			const Vector3f& Pos,
...
namespace ogl
{
	...
	void
	BasicCamera::SetPosition(float x, float y, float z)
	{
		m_pos.x = x;
		m_pos.y = y;
		m_pos.z = z;
	...
	void
	BasicCamera::SetPosition(const Vector3f& pos)
	{
		SetPosition(pos.x, pos.y, pos.z);
	}

	void
	BasicCamera::SetTarget(float x, float y, float z)
	{
		m_target.x = x;
		m_target.y = y;
		m_target.z = z;
	...
	void
	BasicCamera::SetTarget(const Vector3f& target)
	{
		SetTarget(target.x, target.y, target.z);
	}

	bool
	BasicCamera::OnKeyboard(int Key)
	{
	...
	void
	BasicCamera::OnMouse(int x, int y)
	{
	...
	Matrix4f
	BasicCamera::GetViewProjMatrix() const
	{
	...
	Matrix4f
	BasicCamera::GetViewportMatrix() const
	{
	...
	void
	BasicCamera::SetSpeed(float Speed)
	{
		if (Speed <= 0.0f)
		{
